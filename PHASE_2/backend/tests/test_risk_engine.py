...
@pytest.mark.parametrize(
    ("carryover_count", "expected_score", "expected_level"),
    [
        (0, 0.0, RiskLevel.LOW),
        (1, 25.0, RiskLevel.MODERATE),
        (2, 25.0, RiskLevel.MODERATE),
        (3, 60.0, RiskLevel.HIGH),
    ],
)
def test_completed_sprints_use_historical_carryover_risk(carryover_count, expected_score, expected_level):
    """Completed sprints should skip forward-looking risk scoring and use carryover history."""
    start_date = datetime(2025, 1, 1)
    end_date = datetime(2025, 3, 1)
    project_info = ProjectInfo(
        project_name="Completed Sprint Risk",
        sponsor="Test",
        business_unit="Engineering",
        project_manager="PM",
        customer="Customer",
    ...
    project_state = ProjectState(
        project_id="proj-completed-sprint-risk",
        project_info=project_info,
        team=team,
        sprints=sprints,
        work_items=work_items,
    ...
    spillover = SpilloverAnalysisEngine(project_state, metrics.average_item_effort).analyze()
    forecast = ForecastEngine(project_state, metrics, cp_result, spillover).calculate()
    monte_carlo = MonteCarloEngine(project_state, metrics, cp_result, spillover, simulation_count=1000).calculate()
    impact_scores = ImpactScoringEngine(project_state, dag).score()

    risk_engine = RiskEngine(project_state, metrics, cp_result, dag, spillover, forecast, monte_carlo, impact_scores)
    sprint_risks = risk_engine._calculate_sprint_risks()

    assert len(sprint_risks) == 1
    assert sprint_risks[0].risk_score == pytest.approx(expected_score)
    assert sprint_risks[0].risk_level == expected_level
