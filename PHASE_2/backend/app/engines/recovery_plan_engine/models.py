...
class RecoveryPlanScore:
    ...
    composite_score: float  # Weighted formula: 0.45*probability + 0.30*(delay_recovered_fraction) + 0.15*(1-risk) - 0.10*complexity
    baseline_delay_days: float = 0.0  # Expected delay BEFORE this plan's actions were applied (same for every plan in a batch)
    baseline_deadline_probability: float = 0.0  # Deadline probability BEFORE this plan's actions were applied
    baseline_risk_score: float = 0.0  # Overall risk score BEFORE this plan's actions were applied
    meaningful_improvement: bool = False  # True only if actions_required > 0 AND the plan measurably beats baseline (see plan_scorer.py thresholds)
