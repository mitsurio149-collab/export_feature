...
class SPOFDetector:
    ...
    def detect(self) -> List[OpportunitySignal]:
        ...
        for sole_resource_id, critical_items in resource_to_items.items():
            ...
            # If cross_train_backup already ran for this resource, some other team
            # member now covers_skill() each of these skills (recorded via
            # SkillCoverage(proficiency=BACKUP)). Without this check, the applicator's
            # mutation is written but SPOFDetector re-flags the identical signal on
            # the very next pass -- the resilience benefit never actually registers.
            uncovered_skills = [
                skill for skill in skill_names
                if not any(
                    resource.resource_id != sole_resource_id and resource.covers_skill(skill)
                    for resource in self.project_state.team
                )
            ]
            if skill_names and not uncovered_skills:
                continue  # every critical skill this resource holds now has a backup — SPOF resolved
...
class ScheduleDetector:
    ...
    def detect(self) -> List[OpportunitySignal]:
        ...
        if scope_growth_hours > 0:
            # Scope growth percentage must be expressed relative to the original
            # baseline estimate, not relative to remaining effort. Dividing by
            # remaining effort produces an inflated figure (406h / 734h = 55.3%)
            # that contradicts the forecast panel (406h / 1510h = 26.9%).
            # Use total_effort as the denominator to match forecast_engine.py.
            _total_baseline = sum(
                float(wi.estimated_effort_hrs or 0.0)
                for wi in self.project_state.work_items
            )
            context: Dict[str, Any] = {
                "scope_inflation_hours": round(scope_growth_hours, 2),
                "scope_inflation_pct": round(
                    (scope_growth_hours / _total_baseline * 100.0)
                    if _total_baseline > 0 else 0.0,
                    2
                ),
                "flag": "SCOPE_CREEP",
            }
