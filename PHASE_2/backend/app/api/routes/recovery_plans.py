...
def _recommendation_to_api(rec) -> RecommendationInPlanResponse:
    ...
    urgency_val = getattr(rec, "urgency", None)
    urgency_str = urgency_val.value if hasattr(urgency_val, "value") else str(urgency_val) if urgency_val else None
    owner_str = "Unassigned"
    if rec.affected_resource_ids:
        rid = rec.affected_resource_ids[0]
        # project_state is not available here — resolve from the rec's title metadata
        # which already contains the resolved name in parentheses e.g. "(Owner: Ravi Chandran)"
        import re
        m = re.search(r'\(Owner:\s*([^)]+)\)', rec.title or '')
        if m:
            owner_str = m.group(1).strip()
        else:
            # Fall back to humanising the resource_id slug
            owner_str = rid.replace('_', ' ').title()
...
def _score_to_response(score) -> RecoveryPlanScoreResponse:
    ...
    return RecoveryPlanScoreResponse(
        ...
        overall_risk_score=round(score.overall_risk_score),
        ...
        baseline_delay_days=round(score.baseline_delay_days, 2),
        baseline_deadline_probability=round(score.baseline_deadline_probability, 4),
        baseline_risk_score=round(score.baseline_risk_score),
    )
